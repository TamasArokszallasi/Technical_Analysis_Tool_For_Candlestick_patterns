    ...
    for pattern_name, (expectation, pattern_function) in pattern_expectations.items():
        ...
        for i in range(len(df)):
            if pattern_detection[i] != 0:
                ...
                detected_patterns.append([df['Date'][i], pattern_name, expectation, outcome])

    # Convert the list to a DataFrame
    patterns_df = pd.DataFrame(detected_patterns, columns=['Date', 'Pattern', 'Signal', 'Outcome'])
    ...
    with ExcelWriter(filename) as writer:
        for pair, period, start_date, end_date in zip(pairs, periods, start_dates, end_dates):
            try:
                ...
                df.to_excel(writer, sheet_name=f'{pair}_raw', index=False)  # Save raw data in a separate sheet
                analyzed_df = analyze_data(df)
                analyzed_df.to_excel(writer, sheet_name=f'{pair}_analyzed', index=False)  # Save analyzed data in another sheet
