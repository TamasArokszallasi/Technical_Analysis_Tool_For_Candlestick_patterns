...
def main():
    try:
        ...
        # Determine Bollinger Bands signals
        print("Determining Bollinger Bands signals...")
        data['BB_Signal'] = ''
        data['BB_Signal'] = ((data['Close'] > data['BBU_20_2.0']) | (data['Close'] < data['BBL_20_2.0'])).astype(int).diff().fillna(0)
        data['BB_Signal'] = data['BB_Signal'].map({1: 'Bullish', -1: 'Bearish', 0: ''})

        # Print the first few rows with signals to verify
        print("First few rows with signals:")
        print(data[['MACD_12_26_9', 'MACDs_12_26_9', 'MACD_Signal', 'EMA_20', 'EMA_50', 'EMA_200', 'EMA_Signal', 'SMA_20', 'SMA_50', 'SMA_200', 'SMA_Signal', 'BBL_20_2.0', 'BBM_20_2.0', 'BBU_20_2.0', 'BB_Signal']].head())
